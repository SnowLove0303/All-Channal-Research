...
处理橘鸦Juya或其他UP主的AI早报视频时，必须按以下标准格式输出Markdown文档。

### 文件命名规范
```
AI早报_{YYYY-MM-DD}_{UP主名}.md
```
示例：`AI早报_2026-09-01_橘鸦Juya.md`
...
```
1. 获取UP主最新视频列表（按发布时间排序）
2. 检查是否有今日发布的视频
   - 有 → 处理今日视频
   - 无 → 获取并处理最新一条视频（标注为"最新"而非"今日"）
3. 获取字幕/转录（subtitle → whisper转录兜底）
4. 解析转录内容，提取新闻要点
5. 按标准格式生成Markdown文档
6. 保存为 AI早报_{日期}_{UP主}.md
7. 发送给用户
```
...
| 情况 | 处理方式 | 标题标注 |
|------|----------|----------|
| 今日有新视频 | 直接处理 | `AI早报 2026-09-02（周三）` |
| 今日无新视频 | 处理最新一条 | `AI早报（最新）2026-09-01_橘鸦Juya` |
| UP主超过3天未更新 | 提醒用户 | `⚠️ 该UP主已X天未更新` |

### 新闻提取规则

1. **时间标记**：保留转录中的时间戳用于定位
2. **要点提炼**：从转录文本中提取核心信息点
3. **去重合并**：相同话题合并为一条新闻
4. **数据标注**：关键数字、日期、名称加粗
5. **分类整理**：按重要性排序，重大发布优先

### 参考示例

完整示例文件位于 `examples/` 目录：

| 文件 | 说明 |
|------|------|
| `AI早报_2026-09-01_橘鸦Juya.md` | 橘鸦Juya 标准早报输出示例（16条新闻） |
| `AI日报_2026-08-31_黑鸭Heya.md` | 黑鸭Heya 日报输出示例（6条新闻） |
| `transcript_BV1tVtM6CEr3.txt` | 橘鸦Juya 转录文本示例（含时间戳） |
| `transcript_BV1k1t86FELv.txt` | 黑鸭Heya 转录文本示例 |

**已收录UP主**：
| UP主 | UID | 风格 |
|------|-----|------|
| 橘鸦Juya | 285286947 | 详细版，16+条新闻，客观陈述 |
| 黑鸭Heya | 3706929260006322 | 精简版，5-8条新闻，标题党风格 |
| 一只AI风向标 | 3706990266157265 | 分类版，按板块整理（AI动态/基础设施/机器人/排行榜） |

**示例要点**：
- 标题格式：`# AI 早报 2026-09-01（周二）` 或 `# AI 日报 2026-08-31（周日）`
- 元信息：`**来源**: 橘鸦Juya - BV1tVtM6CEr3`
- 新闻条目：`### 1. DeepSeek-V4-Flash-Vision-Exp 权重发布`
- 数据表格：包含播放量、点赞数、弹幕数、评论数、收藏数
