...
function makeBlocks({ lookup, report, transcript, note }) {
  ...
  const blocks = [
    ...
    bullet(`发布时间: ${video.published_at || video.published_date || "unknown"}`),
    ...
    h2("2. 执行结果"),
    bullet(`运行状态: ${report.success ? "成功" : "失败"}`),
